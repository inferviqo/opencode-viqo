...
# パッケージマネージャー
npm i -g viqo-ai@latest        # bun/pnpm/yarn でもOK
scoop install viqo             # Windows
choco install viqo             # Windows
brew install Inferviqo/tap/viqo # macOS と Linux（推奨。常に最新）
brew install viqo              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S viqo            # Arch Linux (Stable)
paru -S viqo-bin               # Arch Linux (Latest from AUR)
mise use -g viqo               # どのOSでも
nix run nixpkgs#viqo           # または github:Inferviqo/viqo で最新 dev ブランチ
```
...
Viqo はデスクトップアプリとしても利用できます。[releases page](https://github.com/Inferviqo/viqo/releases) から直接ダウンロードするか、[inferviqo.com/download](https://inferviqo.com/download) を利用してください。
...
1. `$VIQO_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.viqo/bin` - デフォルトのフォールバック
...
Viqo には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://inferviqo.com/docs/agents) の詳細はこちら。

### ドキュメント

Viqo の設定については [**ドキュメント**](https://inferviqo.com/docs) を参照してください。

### コントリビュート

Viqo に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### Viqo の上に構築する

Viqo に関連するプロジェクトで、名前に "viqo"（例: "viqo-dashboard" や "viqo-mobile"）を含める場合は、そのプロジェクトが Viqo チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/viqo) | [X.com](https://x.com/viqo)
