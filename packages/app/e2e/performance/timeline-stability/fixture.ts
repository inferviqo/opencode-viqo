...
import { Event } from "@viqo-ai/schema/event"
import { SessionStatusEvent } from "@viqo-ai/schema/session-status-event"
import { SessionV1 } from "@viqo-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockViqoServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/Viqo/TimelineStability"
...
export const model = { providerID: "viqo", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("viqo.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["viqo"],
    default: { providerID: "viqo", modelID: "claude-opus-4-6" },
  }
}
