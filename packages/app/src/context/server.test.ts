  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://viqo.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://viqo.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://viqo.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://viqo.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://viqo.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://viqo.example.com"),
      ),
    ).toEqual({
