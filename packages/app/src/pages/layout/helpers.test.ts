...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("viqo://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("viqo://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("viqo://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("viqo://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("viqo://open-project")).toBeUndefined()
    expect(parseDeepLink("viqo://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "viqo://open-project?directory=/a",
      "viqo://other?directory=/b",
      "viqo://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("viqo://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("viqo://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("viqo://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("viqo://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "viqo://new-session?directory=/a",
      "viqo://open-project?directory=/b",
      "viqo://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __VIQO__: {
        deepLinks: ["viqo://open-project?directory=/a"],
      },
    } as unknown as Window & { __VIQO__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["viqo://open-project?directory=/a"])
