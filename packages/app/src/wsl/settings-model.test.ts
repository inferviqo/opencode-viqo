...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Viqo needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/viqo",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Viqo is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        viqoChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.viqo/bin/viqo",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-viqo")
  })
