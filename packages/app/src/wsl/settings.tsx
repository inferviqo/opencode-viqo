    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.viqoChecks[item.config.distro]
          const viqoAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-viqo" && wsl.data.job.distro === item.config.distro
          return (
