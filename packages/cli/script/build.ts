...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      VIQO_CLI_NAME: `'${binary}'`,
      VIQO_MODELS_DEV: modelsData,
      VIQO_CHANNEL: `'${Script.channel}'`,
      VIQO_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/Inferviqo/viqo.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
