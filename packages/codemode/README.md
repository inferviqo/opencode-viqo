...
```ts
import { CodeMode, Tool } from "@viqo-ai/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@viqo-ai/codemode"
...
const runtime = CodeMode.make({ tools: { viqo: api.tools } })
...
The default budget is 2,000 estimated tokens (characters / 4, the same heuristic Viqo uses). Override it when constructing a runtime:
...
No limit has a default, on purpose: execution budgets are host policy, not library policy - a host that wants a bound sets one; a host that can interrupt the execution fiber (as Viqo does on user cancel) may set no timeout, and a host with its own tool-output truncation (as Viqo has) may leave `maxOutputBytes` unset. A host with neither should set `maxOutputBytes`, or oversized results silently flood model context.
...
```ts
import { toolError } from "@viqo-ai/codemode"
