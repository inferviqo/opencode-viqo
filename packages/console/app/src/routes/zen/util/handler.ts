...
import { and, Database, eq, isNull, lt, or, sql } from "@viqo-ai/console-core/drizzle/index.js"
import { KeyTable } from "@viqo-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@viqo-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@viqo-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@viqo-ai/console-core/util/date.js"
import { Identifier } from "@viqo-ai/console-core/identifier.js"
import { Billing } from "@viqo-ai/console-core/billing.js"
import { Actor } from "@viqo-ai/console-core/actor.js"
import { WorkspaceTable } from "@viqo-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@viqo-ai/console-core/model.js"
import { Subscription } from "@viqo-ai/console-core/subscription.js"
import { BlackData } from "@viqo-ai/console-core/black.js"
import { UserTable } from "@viqo-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@viqo-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@viqo-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@viqo-ai/console-core/lite.js"
import { Resource } from "@viqo-ai/console-resource"
...
import { Workspace } from "@viqo-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-viqo-session") ?? ""
    const requestId = input.request.headers.get("x-viqo-request") ?? ""
    const ocClient = input.request.headers.get("x-viqo-client") ?? ""
    const projectId = input.request.headers.get("x-viqo-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://inferviqo.com/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-viqo-request")
            headers.delete("x-viqo-session")
            headers.delete("x-viqo-project")
            headers.delete("x-viqo-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-viqo-endpoint-id")
        const resEndpointModelId = res.headers.get("x-viqo-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://inferviqo.com/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://inferviqo.com/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://inferviqo.com/workspace/${authInfo.workspaceID}/members`
