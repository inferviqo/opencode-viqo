...
export class Service extends Context.Service<Service, Interface>()("@viqo/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["viqo.json", "viqo.jsonc"]
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".viqo")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".viqo").toReversed()
    ...
    // global config, project files, then `.viqo` files.
