...
import type { IntegrationOAuthMethodRegistration } from "@viqo-ai/plugin/v2/effect/integration"
import { define } from "@viqo-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.viqo.ai"
const clientID = "viqo-cli"
...
export const ViqoPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "viqo",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ViqoPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("viqo")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Viqo provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("viqo", (integration) => {
        integration.name = "Viqo"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "viqo", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("viqo")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("viqo")
      ...
      const item = catalog.provider.get(ProviderV2.ID.viqo)
      if (!item) return
      const hasKey = Boolean(process.env.VIQO_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("viqo")),
