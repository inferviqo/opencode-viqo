...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@viqo/acme")).toBe("@viqo/acme")
    expect(Npm.sanitize("@viqo/acme@1.0.0")).toBe("@viqo/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/viqo/acme.git"
    const expected = win ? "acme@git+https_//github.com/viqo/acme.git" : spec
