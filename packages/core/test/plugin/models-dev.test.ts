...
import { ModelsDevPlugin } from "@viqo-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.VIQO_MODELS_PATH,
          disabled: Flag.VIQO_DISABLE_MODELS_FETCH,
        }
        Flag.VIQO_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.VIQO_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.VIQO_MODELS_PATH = previous.path
          Flag.VIQO_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
