...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.viqo.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `viqo.settings.dat` is special: it maps to the `viqo.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"viqo.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // viqo.settings.dat → the electron settings store ("viqo.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "viqo.global.dat").
  const storeName = filename === "viqo.settings.dat" ? "viqo.settings" : filename
