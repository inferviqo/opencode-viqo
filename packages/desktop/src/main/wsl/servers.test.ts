...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectViqoVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectViqoVersion("1.14.35", "1.16.2")).toThrow(
    "Viqo update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Viqo", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.viqo/bin/viqo",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, viqoChecks: {} })
})
...
test("ignores stale background Viqo checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().viqoChecks).toEqual({})
})

test("ignores stale startup Viqo checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Viqo", async () => {
  persistedServers = []
  ...
  expect(viqo).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().viqoChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Viqo in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const viqo: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(viqo).toEqual(["Debian"])
  expect(Object.keys(controller.getState().viqoChecks)).toEqual(["Debian"])
})
