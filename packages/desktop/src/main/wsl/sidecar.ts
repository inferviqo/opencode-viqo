...
import { type WslCommandLine, resolveWslViqo, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const viqo = await resolveWslViqo(distro)
  if (!viqo) throw new Error(`Viqo is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export VIQO_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export VIQO_CLIENT=desktop",
    `export VIQO_SERVER_USERNAME=${shellEscape(username)}`,
    `export VIQO_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(viqo)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
