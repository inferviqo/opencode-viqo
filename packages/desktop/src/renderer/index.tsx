...
import type { UpdaterState } from "@viqo-ai/app/updater"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.VIQO_CHANNEL === "prod" &&
...
const deepLinkEvent = "viqo:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__VIQO__ ??= {}
  const pending = window.__VIQO__.deepLinks ?? []
  window.__VIQO__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `viqo.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://inferviqo.com/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("viqo.global.dat").getItem("language")
