...
import { PromptInput } from "@viqo-ai/schema/prompt-input"
import { Session } from "@viqo-ai/schema/session"
import { Project } from "@viqo-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@viqo-ai/schema/schema"
...
import { SessionEvent } from "@viqo-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Viqo process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Viqo process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
