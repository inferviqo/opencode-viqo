...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("viqo-find")
  api.delete("viqo-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("viqo-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("viqo-find", new Highlight(...rest))
    return true
  }
