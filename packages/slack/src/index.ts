...
import { createOpencode, type ToolPart } from "@viqo-ai/sdk"
...
console.log("🚀 Starting viqo server...")
const viqo = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await viqo.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new viqo session...")
    const { client, server } = viqo
    ...
    console.log("✅ Created viqo session:", createResult.data.id)
  ...
  console.log("📝 Sending to viqo:", message.text)
