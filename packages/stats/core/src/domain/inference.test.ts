...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve viqo route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "viqo")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "viqo")).toBe("openai")
    expect(statProvider("big-pickle", "", "viqo")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "viqo"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep viqo as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "viqo"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "viqo"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep viqo or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "viqo"), country: "US" })).toMatchObject([
