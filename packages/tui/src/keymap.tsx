...
export const VIQO_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const VIQO_MODE_KEY = "viqo.mode"

export const ViqoKeymapProvider = KeymapProvider
export const useViqoKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ViqoModeStack = ReturnType<typeof createViqoModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ViqoModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createViqoModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(VIQO_MODE_KEY, VIQO_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(VIQO_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(VIQO_MODE_KEY, stack.at(-1)?.mode ?? VIQO_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? VIQO_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(VIQO_MODE_KEY, undefined)
...
export function useViqoModeStack() {
  return getViqoModeStack(useViqoKeymap())
}

export function getViqoModeStack(keymap: OpenTuiKeymap) {
...
export function registerViqoKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createViqoModeStack(keymap)
