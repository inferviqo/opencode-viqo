import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@viqo-ai/plugin/tui"
...
import type { useViqoKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useViqoKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getViqoModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getViqoModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
