...
import { getViqoModeStack, VIQO_BASE_MODE, ViqoKeymapProvider, registerViqoKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerViqoKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when viqo mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getViqoModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getViqoModeStack(keymap).push("autocomplete")
