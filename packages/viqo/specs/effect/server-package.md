...
Practical reference for a future `packages/server` split after the viqo
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/viqo`.
...
Until enough shared service code lives outside `packages/viqo`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/viqo`

It should not import `packages/viqo` services while `packages/viqo`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on viqo-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/viqo` implementation details.
