...
Trim `packages/viqo/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/viqo/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/viqo`.
- `[x]` `bun typecheck` from `packages/viqo`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/viqo/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/viqo`.
- `[x]` `bun dev generate > /tmp/viqo-openapi.json` from `packages/viqo`.
...
- Edit `packages/viqo/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/viqo-openapi.json` from `packages/viqo`.
...
- Check source schemas in files like `packages/viqo/src/session/schema.ts`, `packages/viqo/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/viqo/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/viqo/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
