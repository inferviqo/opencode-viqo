...
Delete `packages/viqo/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/viqo/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/viqo/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/viqo`
- TUI plugin loader tests in `packages/viqo` if runtime plugin API wiring changed
