...
import type { AssistantMessage, Message, ViqoClient, SessionMessageResponse } from "@viqo-ai/sdk/v2"
...
export const AuthMethodID = "viqo-login"
...
export class Service extends Context.Service<Service, Interface>()("@viqo/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `viqo auth login` in the terminal",
      name: "Login with viqo",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: ViqoClient) {
...
function makeUsageService(sdk: ViqoClient) {
...
async function loadDirectorySnapshot(sdk: ViqoClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, viqo provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const viqoProvider = providers[ProviderV2.ID.make("viqo")]
  const viqoModel = viqoProvider ? Provider.sort(Object.values(viqoProvider.models))[0] : undefined
  if (viqoProvider && viqoModel) return { providerID: viqoProvider.id, modelID: viqoModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Viqo service failure", service })
}
