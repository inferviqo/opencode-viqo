import type { PermissionV1 } from "@viqo-ai/core/v1/permission"
import { FSUtil } from "@viqo-ai/core/fs-util"
// CLI entry point for `viqo run` and `viqo --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`viqo --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`viqo --mini --attach`): connects to a running
//      viqo server and runs interactive mode against it.
//
...
import { createViqoClient, type ViqoClient, type ToolPart } from "@viqo-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ViqoClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run viqo with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running viqo server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to VIQO_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to VIQO_SERVER_USERNAME or 'viqo')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ViqoClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ViqoClient, sessionID: string) {
      ...
      async function current(sdk: ViqoClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ViqoClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ViqoClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ViqoClient) {
        ...
        async function loop(client: ViqoClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createViqoClient({
        baseUrl: "http://viqo.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
