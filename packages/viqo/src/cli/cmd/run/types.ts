// Shared type vocabulary for the direct interactive mode (`viqo --mini`).
//
...
import type { ViqoClient, PermissionRequest, QuestionRequest, ToolPart } from "@viqo-ai/sdk/v2"
import type { TuiConfig } from "@viqo-ai/tui/config"
...
type PromptModel = Parameters<ViqoClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ViqoClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ViqoClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ViqoClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ViqoClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ViqoClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ViqoClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ViqoClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ViqoClient["question"]["reject"]>[0]
