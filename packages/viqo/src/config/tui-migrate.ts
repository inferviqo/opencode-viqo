...
const TUI_SCHEMA_URL = "https://inferviqo.com/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from viqo.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const viqo = await viqoFiles(input)
  for (const file of viqo) {
...
async function viqoFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "viqo"),
    ...(await Filesystem.findUp(["viqo.json", "viqo.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "viqo"))
  }
  if (Flag.VIQO_CONFIG) files.push(Flag.VIQO_CONFIG)
