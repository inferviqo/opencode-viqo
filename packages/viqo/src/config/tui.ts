...
import { isRecord } from "@viqo-ai/tui/util/record"
...
import { TuiKeybind } from "@viqo-ai/tui/config/keybind"
...
import { TuiConfig } from "@viqo-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@viqo/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old viqo.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.viqo`
  // folders between cwd and home, and VIQO_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.VIQO_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit VIQO_TUI_CONFIG override, if set.
  if (Flag.VIQO_TUI_CONFIG) {
    const configFile = Flag.VIQO_TUI_CONFIG
  ...
  // 4. `.viqo` directories (and VIQO_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".viqo") || dir === Flag.VIQO_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".viqo") && dir !== Flag.VIQO_CONFIG_DIR) continue
