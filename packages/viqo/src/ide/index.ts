...
import { IdeEvent } from "@viqo-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["VIQO_CALLER"] === "vscode" || process.env["VIQO_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.viqo"], {
    nothrow: true,
  })
