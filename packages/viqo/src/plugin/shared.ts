...
export const DEPRECATED_PLUGIN_PACKAGES = ["viqo-openai-codex-auth", "viqo-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, viqoVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(viqoVersion) || semver.major(viqoVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.viqo
  if (typeof range !== "string") return
  if (!semver.satisfies(viqoVersion, range)) {
    throw new Error(`Plugin requires viqo ${range} but running ${viqoVersion}`)
  }
}
