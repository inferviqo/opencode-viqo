...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@viqo-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@viqo-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@viqo-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@viqo-ai/tui/context/theme"
import { Global } from "@viqo-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@viqo-ai/core/util/flock"
import { Flag } from "@viqo-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@viqo-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@viqo-ai/core/v1/config/plugin"
import { createCommandShim } from "@viqo-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@viqo-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".viqo"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".viqo", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".viqo", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.VIQO_PURE ? [] : pluginOrigins
    if (Flag.VIQO_PURE && pluginOrigins.length) {
    }
