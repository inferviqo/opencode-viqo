...
import type { LLMClientShape } from "@viqo-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("viqo"))
    return { type: "unsupported", reason: "provider is not openai, viqo, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @viqo-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains viqo-owned. The native runtime only adapts
      // the @viqo-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
