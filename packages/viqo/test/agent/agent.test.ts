...
it.instance("plan agent denies edits except .viqo/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".viqo/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".viqo", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.VIQO_TEST_HOME
      process.env.VIQO_TEST_HOME = test.directory
