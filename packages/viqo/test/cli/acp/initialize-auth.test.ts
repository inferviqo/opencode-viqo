...
describe("viqo acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ viqo }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ viqo }))
        ...
        expect(initialized.agentInfo?.name).toBe("Viqo")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ viqo })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("viqo-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "viqo-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.VIQO_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("viqo-login")
