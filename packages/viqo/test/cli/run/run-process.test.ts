// Subprocess integration tests for `viqo run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `viqo.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `VIQO_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("viqo run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("say hi")
        viqo.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* viqo.run("think", { extraArgs: ["--thinking"] })
        viqo.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* viqo.run("think again")
        viqo.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("say hi", { format: "json" })
        viqo.expectExit(result, 0)

        const events = viqo.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const result = yield* viqo.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const result = yield* viqo.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* viqo.run("request permission", { permission: { bash: "ask" } })
        viqo.expectExit(denied, 0)
        ...
        const allowed = yield* viqo.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        viqo.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* viqo.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        viqo.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, viqo }) =>
      Effect.gen(function* () {
        ...
        const server = yield* viqo.serve()

        const result = yield* viqo.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, viqo }) =>
      Effect.gen(function* () {
        const result = yield* viqo.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, viqo }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* viqo.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
