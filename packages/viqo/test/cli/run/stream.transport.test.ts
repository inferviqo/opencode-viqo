...
import { ViqoClient, type GlobalEvent } from "@viqo-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ViqoClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ViqoClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ViqoClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ViqoClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ViqoClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://viqo.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ViqoClient["event"]["subscribe"]
    globalEvent?: ViqoClient["global"]["event"]
    promptAsync?: ViqoClient["session"]["promptAsync"]
    status?: ViqoClient["session"]["status"]
    messages?: ViqoClient["session"]["messages"]
    children?: ViqoClient["session"]["children"]
    permissions?: ViqoClient["permission"]["list"]
    questions?: ViqoClient["question"]["list"]
  } = {},
) {
  const client = new ViqoClient()

  const subscribe: ViqoClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ViqoClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ViqoClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ViqoClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ViqoClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ViqoClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ViqoClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ViqoClient["question"]["list"] = input.questions ?? (() => ok([]))
