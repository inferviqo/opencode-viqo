...
describe("viqo read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // VIQO_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["mcp", "list"])
        viqo.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["providers", "list"])
        viqo.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["models"])
        viqo.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["agent", "list"])
        viqo.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh VIQO_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["session", "list"])
        viqo.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["stats"])
        viqo.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under VIQO_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ viqo }) =>
      Effect.gen(function* () {
        const r = yield* viqo.spawn(["db", "path"])
        viqo.expectExit(r, 0, "db path")
