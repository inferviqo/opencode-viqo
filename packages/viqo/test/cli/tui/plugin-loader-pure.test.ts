...
`,
  ...
  const pure = process.env.VIQO_PURE
  const meta = process.env.VIQO_PLUGIN_META_FILE
  process.env.VIQO_PURE = "1"
  process.env.VIQO_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.VIQO_PURE
    } else {
      process.env.VIQO_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.VIQO_PLUGIN_META_FILE
    } else {
      process.env.VIQO_PLUGIN_META_FILE = meta
    }
  }
})
