...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ viqo }) =>
    Effect.gen(function* () {
      const result = yield* viqo.spawn(["--replay-limit", "10"])

      viqo.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ viqo }) =>
    Effect.gen(function* () {
      const result = yield* viqo.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ viqo }) =>
    Effect.gen(function* () {
      const result = yield* viqo.spawn(["--mini", "--port", "4096"])
