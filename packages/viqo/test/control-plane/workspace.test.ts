...
import { EventSequenceTable } from "@viqo-ai/core/event/sql"
...
import { WorkspaceTable } from "@viqo-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  VIQO_AUTH_CONTENT: process.env.VIQO_AUTH_CONTENT,
  VIQO_EXPERIMENTAL_WORKSPACES: process.env.VIQO_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.VIQO_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@viqo.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.VIQO_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=viqo-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.VIQO_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.VIQO_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.VIQO_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=viqo-test")
