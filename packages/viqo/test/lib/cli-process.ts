// Subprocess test harness for the viqo CLI. Spawns the real binary against
...
//
// Configuration flows through viqo's built-in test affordances:
//   - VIQO_CONFIG_CONTENT      : provider config inline, no files to find
//   - VIQO_TEST_HOME           : pins os.homedir() → tmpdir
//   - VIQO_DISABLE_PROJECT_CONFIG : skip walking up for viqo.json
//   - VIQO_PURE                : skip external plugin discovery + install
//   - VIQO_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `viqo.run` is fully wired. The shape supports adding more
// builders (`viqo.serve(opts)`, `viqo.acp(opts)`, `viqo.auth(...)`)
...
import { AppProcess } from "@viqo-ai/core/process"
...
const viqoRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(viqoRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    VIQO_CONFIG_CONTENT: configJson,
    VIQO_DISABLE_PROJECT_CONFIG: "1",
    VIQO_PURE: "1",
    VIQO_DISABLE_AUTOUPDATE: "1",
    VIQO_DISABLE_AUTOCOMPACT: "1",
    VIQO_DISABLE_MODELS_FETCH: "1",
    VIQO_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `viqo run`. New flags should
...
// `viqo serve` is a long-lived process — it never exits on its own.
...
// `viqo acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `viqo serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `viqo acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("viqo.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          VIQO_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("viqo.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("viqo.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "viqo server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `viqo serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("viqo.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const viqo: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, viqo })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `viqo.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "viqo") {
...
// every call site. Use this for any test that needs the viqo CLI fixture.
//
...
// resources (e.g. `viqo.serve`) without an extra `Effect.scoped` wrapper —
