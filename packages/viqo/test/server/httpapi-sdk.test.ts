...
import { createViqoClient } from "@viqo-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createViqoClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.VIQO_SERVER_PASSWORD = input?.password
      Flag.VIQO_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".viqo", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-viqo-directory")).toBe(false)
        expect(request!.headers.has("x-viqo-workspace")).toBe(false)
      }),
    ),
  )
