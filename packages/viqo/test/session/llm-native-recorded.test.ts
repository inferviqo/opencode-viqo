...
import { SessionV1 } from "@viqo-ai/core/v1/session"
import { ModelsDev } from "@viqo-ai/core/models-dev"
import { HttpRecorder } from "@viqo-ai/http-recorder"
import { HttpRecorderInternal } from "@viqo-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@viqo-ai/llm"
import { RequestExecutor } from "@viqo-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.viqo.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.VIQO_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["viqo", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("VIQO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("VIQO_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["viqo", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["viqo", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.VIQO_RECORD_CONSOLE_TOKEN && process.env.VIQO_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.viqo,
        name: "Viqo Zen",
        env: ["VIQO_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.VIQO_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.VIQO_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.VIQO_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["viqo", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("VIQO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("VIQO_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("VIQO_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set VIQO_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the viqo LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "viqo.json"),
      JSON.stringify({ $schema: "https://inferviqo.com/config.json", ...scenario.config(model) }),
    ),
  )
