...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.VIQO_WEBSEARCH_PROVIDER

    try {
      process.env.VIQO_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.VIQO_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.VIQO_WEBSEARCH_PROVIDER
      else process.env.VIQO_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for viqo or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.viqo, { exa: false, parallel: false })).toBe(true)
