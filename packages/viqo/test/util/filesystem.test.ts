...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "viqo.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "viqo.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "viqo.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "viqo.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["viqo.json", "viqo.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "viqo.json"),
        path.join(project, "viqo.jsonc"),
      ])
    })
  })
