...
أنشئ الملف `.viqo/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Viqo أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Viqo:
...
- على مستوى النظام: `~/.config/viqo/commands/`
- لكل مشروع: `.viqo/commands/`
...
يتضمن viqo عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
