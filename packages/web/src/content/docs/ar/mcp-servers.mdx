...
يمكنك إضافة أدوات خارجية إلى Viqo باستخدام _Model Context Protocol_، أو MCP. يدعم Viqo خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Viqo](https://inferviqo.com/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/viqo` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Viqo تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Viqo بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Viqo المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `viqo mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Viqo الرموز بشكل آمن في `~/.local/share/viqo/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Viqo إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Viqo مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Viqo بحساب Sentry الخاص بك.
