---
title: المزوّدون
description: استخدام أي موفّر LLM في Viqo.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Viqo ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Viqo.
...
في `~/.local/share/viqo/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Viqo.
...
Viqo Zen هي قائمة نماذج يوفّرها فريق Viqo وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Viqo. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Viqo Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر viqo، ثم انتقل إلى [inferviqo.com/auth](https://inferviqo.com/auth).
...
يعمل مثل أي مزوّد آخر في Viqo واستخدامه اختياري بالكامل.

---

## Viqo Go

Viqo Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Viqo، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Viqo.

1. شغّل الأمر `/connect` في TUI، واختر `Viqo Go`، ثم انتقل إلى [inferviqo.com/auth](https://inferviqo.com/zen).
...
لاستخدام Amazon Bedrock مع Viqo:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل viqo:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `viqo.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Viqo غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين viqo لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل viqo بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات viqo.
...
تخزين الرمز في مساحة تخزين مصادقة viqo.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Viqo نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Viqo
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`viqo.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع viqo:
...
لاستخدام Google Vertex AI مع Viqo:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر viqo، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session) لتسجيل كل محادثة Viqo تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Viqo مدرجة كجلسة مستقلة.
...
يمكنك ضبط viqo لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط viqo لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط viqo لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Viqo. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/viqo) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Viqo.
...
6. **مهم**: قبل استخدام نماذج السحابة في Viqo، يجب سحب معلومات النموذج محليا:
...
Viqo Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Viqo. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Viqo Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Viqo Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Viqo.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات viqo.
...
5. يمكنك أيضا تخصيصها عبر إعدادات viqo. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات viqo. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع viqo. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `viqo.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Viqo بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `viqo auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات viqo وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات viqo.
