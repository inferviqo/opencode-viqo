---
title: الخادم
description: تفاعل مع خادم viqo عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `viqo serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل viqo استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `viqo.local` |
...
عيّن `VIQO_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `viqo`، أو عيّن `VIQO_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `viqo serve` و `viqo web`.
...
عند تشغيل `viqo` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم viqo للتفاعل مع viqo برمجيا.
:::

تتيح هذه البنية لـ viqo دعم عدة عملاء وتمكّنك من التفاعل مع viqo برمجيا.

يمكنك تشغيل `viqo serve` لبدء خادم مستقل. إذا كانت واجهة viqo في terminal (TUI)
قيد التشغيل، فسيبدأ `viqo serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Viqo لـ [IDE](/docs/ide).
...
يعرض خادم viqo واجهات API التالية.
