...
مع Viqo يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Viqo سمتنا `viqo`.
...
- يريدون أن يطابق Viqo مظهر terminal لديهم
...
يدعم Viqo نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/viqo/themes/*.json` أو `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.viqo/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.viqo/themes/*.json`
