---
title: Konfiguracija
description: Korištenje Viqo JSON konfiguracije.
---

Možete konfigurirati Viqo koristeći JSON konfiguracijski fajl.

---

## Format

Viqo podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/viqo`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/viqo/viqo.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`VIQO_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`viqo.json` u projektu) - postavke specifične za projekat
5. **`.viqo` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`VIQO_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.viqo` i `~/.config/viqo` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/viqo` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Viqo konfiguraciju u `~/.config/viqo/viqo.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/viqo/tui.json`.
...
Dodajte `viqo.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Viqo pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `VIQO_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `VIQO_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.viqo` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.viqo`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`inferviqo.com/config.json`**](https://inferviqo.com/config.json).

TUI konfiguracija koristi [**`inferviqo.com/tui.json`**](https://inferviqo.com/tui.json).
...
Koristite `VIQO_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `viqo.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `viqo serve` i `viqo web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Viqo server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `viqo.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Viqo konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Viqo pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/viqo/agents/` ili `.viqo/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Viqo će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`viqo run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/viqo/commands/` ili `.viqo/commands/`. [Saznajte više ovdje](/docs/commands).
...
Viqo će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Viqo **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Viqo sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.viqo/plugins/` ili `~/.config/viqo/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Viqo da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
