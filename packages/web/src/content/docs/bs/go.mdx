...
Viqo Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Viqo-u. Pretplatite se na Viqo Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Viqo.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Viqo-om.
:::
...
Viqo Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Viqo Go radi kao bilo koji drugi provajder u Viqo-u.

1. Prijavite se na **<a href={console}>Viqo Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Viqo Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Viqo Go.
:::
...
Viqo Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Viqo konfiguraciji
koristi format `viqo-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`viqo-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo Viqo Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Viqo koristite i bilo kojeg drugog provajdera.
