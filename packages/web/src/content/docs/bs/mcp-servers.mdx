...
Možete dodati vanjske alate u Viqo koristeći _Model Context Protocol_, ili MCP. Viqo podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Viqo Config](https://inferviqo.com/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/viqo`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Viqo automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Viqo će:
...
Ako server zahtijeva autentifikaciju, Viqo će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `viqo mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Viqo sigurno čuva tokene u `~/.local/share/viqo/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Viqo, zajedno s ugrađenim alatima. Možete njima upravljati kroz Viqo konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Viqo sa vašim Sentry nalogom.
