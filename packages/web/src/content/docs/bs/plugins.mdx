---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Viqo.
---

Dodaci vam omogućavaju da proširite Viqo spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Viqo.
...
- `.viqo/plugins/` - Dodaci na nivou projekta
- `~/.config/viqo/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/viqo/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/viqo/viqo.json`)
2. Konfiguracija projekta (`viqo.json`)
3. Globalni direktorij dodataka (`~/.config/viqo/plugins/`)
4. Direktorij dodataka projekta (`.viqo/plugins/`)
...
Viqo pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Viqo.
...
Spriječite viqo da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u viqo:
...
Pomoćnik `tool` kreira prilagođeni alat koji viqo može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za viqo zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://inferviqo.com/docs/sdk) za detalje.
