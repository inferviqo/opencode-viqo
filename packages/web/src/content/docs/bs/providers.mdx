---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Viqo.
---

import config from "../../../../config.mjs"
export const console = config.console

Viqo koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Viqo konfiguraciji.
...
u `~/.local/share/viqo/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Viqo
config.
...
Viqo Zen je lista modela koje je obezbedio Viqo tim koji su bili
testirano i potvrđeno da dobro radi sa Viqo. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Viqo Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite viqo i idite na [inferviqo.com/auth](https://inferviqo.com/auth).
...
Radi kao i svaki drugi provajder u Viqo i potpuno je opcionalan za korištenje.

---

## Viqo Go

Viqo Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Viqo tim i koji su testirani i verificirani da dobro rade s Viqo-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Viqo Go` i idite na [inferviqo.com/auth](https://inferviqo.com/zen).
...
Da biste koristili Amazon Bedrock s Viqo:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete viqo:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `viqo.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Viqo.
:::
...
Možete konfigurirati viqo za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi viqo ispravno radio.
   :::
...
Također možete dodati modele kroz svoju viqo konfiguraciju.
...
da pohrani token u viqo auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Viqo koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Viqo
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`viqo.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s viqo:
...
Za korištenje Google Vertex AI s Viqo:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće viqo.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem viqoa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session) da automatski prijavite svaki Viqo razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Viqo razgovor naveden kao zasebna sesija.
...
Možete konfigurirati viqo za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati viqo za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati viqo za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Viqo. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/viqo) za detalje.
:::
...
Da biste koristili Ollama Cloud s Viqo:
...
3. Kopirajte API ključ za korištenje u Viqo.
...
6. **Važno**: Prije upotrebe modela oblaka u Viqo, morate lokalno povući informacije o modelu:
...
Viqo Zen je lista testiranih i verifikovanih modela koju je obezbedio Viqo tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Viqo Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Viqo Zen**.

```txt
   /connect
```

3. Unesite svoj Viqo API ključ.
...
Također možete dodati dodatne modele putem vaše viqo konfiguracije.
...
5. Također ih možete prilagoditi putem vaše viqo konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju viqo konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s viqo-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `viqo.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Viqo da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `viqo auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Viqo konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj viqo konfiguraciji.
