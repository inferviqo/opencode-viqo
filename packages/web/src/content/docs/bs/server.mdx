---
title: Server
description: Komunicirajte s viqo serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `viqo serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji viqo klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `viqo.local` |
...
Postavite `VIQO_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `viqo`, ili postavite `VIQO_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `viqo serve` i za `viqo web`.
...
Kada pokrenete `viqo`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite viqo server za programsku interakciju sa viqo-om.
:::

Ova arhitektura omogucava viqo podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `viqo serve` da startate standalone server. Ako je viqo TUI vec pokrenut, `viqo serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Viqo [IDE](/docs/ide) pluginovi.
...
viqo server izlaže sljedece API-je.
