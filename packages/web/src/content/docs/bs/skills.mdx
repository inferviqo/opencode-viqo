...
Agent skills omogucavaju Viqo da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Viqo pretrazuje ove lokacije:

- Konfiguracija projekta: `.viqo/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/viqo/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Viqo ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.viqo/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.viqo/skills/git-release/SKILL.md` ovako:
...
Viqo navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `viqo.json`:
...
**Za ugradene agente** (u `viqo.json`):
