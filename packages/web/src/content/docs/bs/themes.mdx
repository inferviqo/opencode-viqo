...
U Viqo mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Viqo koristi nasu `viqo` temu.
...
Viqo dolazi sa vise ugradenih tema.
...
- Zele da Viqo odgovara izgledu njihovog terminala
...
Viqo podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/viqo/themes/*.json` ili `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.viqo/themes/*.json`
4. **Trenutni radni direktorij** - `./.viqo/themes/*.json`
