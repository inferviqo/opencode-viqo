...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Viqo dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Viqo.
...
:::note
Ovaj alat je dostupan samo kada je `VIQO_EXPERIMENTAL_LSP_TOOL=true` (ili `VIQO_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Viqo provajdera ili kada je varijabla `VIQO_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Viqo:
