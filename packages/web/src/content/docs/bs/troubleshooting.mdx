...
Da biste otklonili probleme s Viqo, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\viqo\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `viqo --log-level DEBUG`.

---

## Pohrana

Viqo pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\viqo`
...
Viqo Desktop pokreće lokalni Viqo server (`viqo-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Viqo` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (ili `~/.config/viqo/viqo.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\viqo\viqo.jsonc`
...
Viqo također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\viqo\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Viqo mogao ponovo izgraditi.

1. Potpuno zatvorite Viqo Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/viqo`
- **Linux**: obrišite `~/.cache/viqo` (ili pokrenite `rm -rf ~/.cache/viqo`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\viqo`

3. Ponovo pokrenite Viqo Desktop.

---

### Rješavanje problema sa vezom na serveru

Viqo Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `viqo.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `VIQO_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `VIQO_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Viqo Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Viqo karakteristikama.

---

### Obavještenja se ne prikazuju

Viqo Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Viqo u postavkama vašeg OS-a, i
...
1. Zatvorite Viqo Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Viqo Desktop aplikacije):

- `viqo.settings.dat` (URL zadanog servera za desktop)
- `viqo.global.dat` i `viqo.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `viqo upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `viqo models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\viqo`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Viqo dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\viqo`

2. Ponovo pokrenite Viqo da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Viqo da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Viqo će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
