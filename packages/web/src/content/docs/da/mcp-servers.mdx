...
Du kan tilføje eksterne værktøjer til Viqo ved hjælp af _Model Context Protocol_ eller MCP. Viqo understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Viqo Config](https://inferviqo.com/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/viqo`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Viqo håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Viqo:
...
Hvis serveren kræver godkendelse, vil Viqo bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `viqo mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Viqo opbevare tokens sikkert i `~/.local/share/viqo/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Viqo-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Viqo til din Sentry-konto.
