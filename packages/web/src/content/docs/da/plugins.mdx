---
title: Plugins
description: Skriv dine egne plugins for at udvide Viqo.
---

Plugins giver dig mulighed for at udvide Viqo ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Viqos standardadfærd.
...
- `.viqo/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/viqo/node_modules/`.
...
1. Global konfiguration (`~/.config/viqo/viqo.json`)
2. Projektkonfiguration (`viqo.json`)
3. Global plugin-mappe (`~/.config/viqo/plugins/`)
4. Projekt plugin bibliotek (`.viqo/plugins/`)
...
Viqo kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En viqo SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide viqo.
...
:::note
Hvis du bruger Viqo desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå viqo i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til viqo:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som viqo kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for viqo sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://inferviqo.com/docs/sdk) for detaljer.
