---
title: Server
description: Interager med viqo-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `viqo serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en viqo-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `viqo.local` |
...
Indstil `VIQO_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `viqo`, eller indstil `VIQO_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `viqo serve` og `viqo web`.
...
Når du kører `viqo` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug viqo-serveren til at interagere med viqo programmatisk.
:::

Denne arkitektur lader viqo understøtte flere klienter og lader dig interagere med viqo programmatisk.

Du kan køre `viqo serve` for at starte en selvstændig server. Hvis du har
viqo TUI kørende, vil `viqo serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Viqo [IDE](/docs/ide) plugins.
...
Viqo-serveren viser følgende API'er.
