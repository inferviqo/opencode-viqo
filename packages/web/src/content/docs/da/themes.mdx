...
Med Viqo kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Viqo vårt eget `viqo`-tema.
...
Viqo støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/viqo/themes/*.json` eller `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.viqo/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.viqo/themes/*.json`
