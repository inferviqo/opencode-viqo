...
For at fejlfinde problemer med Viqo, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\viqo\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `viqo --log-level DEBUG`.

---

## Lagring

viqo gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\viqo`
...
Viqo Desktop kører en lokal Viqo-server (`viqo-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Viqo`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (eller `~/.config/viqo/viqo.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\viqo\viqo.jsonc`
...
Viqo kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\viqo\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Viqo kan genopbygge den.

1. Afslut Viqo Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/viqo`
- **Linux**: slet `~/.cache/viqo` (eller kør `rm -rf ~/.cache/viqo`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\viqo`

3. Start Viqo Desktop på ny.

---

### Løs problemer med serverforbindelse

Viqo Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `viqo.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `VIQO_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `VIQO_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Viqo Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Viqo.

---

### Meddelelser vises ikke

Viqo Desktop viser kun systemvarsler når:

- varsler er aktiveret for Viqo i dine OS-indstillinger, og
...
1. Afslut Viqo Desktop.
2. Find og slet disse filer (de findes i Viqo Desktop-appens datamappe):

- `viqo.settings.dat` (skrivebordsstandardserver URL)
- `viqo.global.dat` og `viqo.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Viqo:
...
For at finde ud af hvilke modeller du har adgang til, kør `viqo models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\viqo`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. viqo installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\viqo`

2. Start viqo på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge viqo til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
viqo vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
