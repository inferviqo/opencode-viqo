...
description: Erstellen Sie Tools, die der LLM in Viqo aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Viqo wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.viqo/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/viqo/tools/` platzieren.
