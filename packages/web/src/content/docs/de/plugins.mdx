---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Viqo zu erweitern.
---

Mit Plugins können Sie Viqo erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Viqo zu ändern.
...
- `.viqo/plugins/` – Plugins auf Projektebene
- `~/.config/viqo/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/viqo/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/viqo/viqo.json`)
2. Projektkonfiguration (`viqo.json`)
3. Globales Plugin-Verzeichnis (`~/.config/viqo/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.viqo/plugins/`)
...
Viqo führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Viqo-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Viqo erweitern können.
...
:::note
Wenn Sie die Viqo-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Viqo `.env`-Dateien liest:
...
Plugins können Viqo auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Viqo aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Viqo zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://inferviqo.com/docs/sdk).
