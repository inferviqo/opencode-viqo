---
title: Regeln
description: Lege eigene Anweisungen fuer viqo fest.
---

Du kannst viqo ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in viqo den Befehl `/init` aus.
...
So kann viqo sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht viqo sie zu erweitern.
...
viqo kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/viqo/AGENTS.md` speichern.
Diese gelten in allen viqo-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Viqo dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/viqo/AGENTS.md` existiert)
...
Beim Start sucht viqo in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/viqo/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `viqo.json` oder in `~/.config/viqo/viqo.json` angeben.
...
viqo parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit viqo.json

Empfohlen ist das `instructions`-Feld in der `viqo.json`:
...
Du kannst viqo explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `viqo.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
