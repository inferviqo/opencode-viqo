---
title: Server
description: Nutze den viqo-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `viqo serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein viqo-Client nutzen kann.
...
Setze `VIQO_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `viqo`, kann aber mit `VIQO_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `viqo serve` und `viqo web`.
...
Wenn du `viqo` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den viqo-Server, um viqo programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `viqo serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `viqo serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Viqo-[IDE](/docs/ide)-Plugins.
...
Der viqo-Server stellt folgende APIs bereit.
