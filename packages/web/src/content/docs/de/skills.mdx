...
Agent Skills erlauben Viqo, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Viqo sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Viqo vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.viqo/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.viqo/skills/git-release/SKILL.md` zum Beispiel so:
...
Viqo listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `viqo.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `viqo.json`):
