...
Mit Viqo kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Viqo das `viqo`-Theme.
...
Viqo liefert mehrere eingebaute Themes mit.
...
- Viqo optisch ans Terminal anpassen wollen
...
Viqo unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/viqo/themes/*.json` oder `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.viqo/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.viqo/themes/*.json`
