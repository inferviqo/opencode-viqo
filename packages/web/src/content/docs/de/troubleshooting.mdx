...
Wenn Viqo Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\viqo\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `viqo --log-level DEBUG`.

---

## Speicher

viqo speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\viqo`
...
Viqo Desktop startet im Hintergrund einen lokalen Viqo-Server (`viqo-cli`-Sidecar).
...
- Nur macOS: `Viqo`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (oder `~/.config/viqo/viqo.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\viqo\viqo.jsonc`
...
Viqo kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\viqo\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Viqo ihn neu aufbauen kann.

1. Beende Viqo Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/viqo`
- **Linux**: loesche `~/.cache/viqo` (oder fuehre aus: `rm -rf ~/.cache/viqo`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\viqo`

3. Starte Viqo Desktop neu.

---

### Server-Verbindungsprobleme beheben

Viqo Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `viqo.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `VIQO_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `VIQO_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Viqo Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Viqo-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Viqo Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Viqo in den OS-Einstellungen aktiviert sind, und
...
1. Beende Viqo Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Viqo Desktop):

- `viqo.settings.dat` (desktop default server URL)
- `viqo.global.dat` und `viqo.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Viqo hast:
...
3. Stelle sicher, dass du die neueste Version hast: `viqo upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `viqo models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\viqo`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. viqo installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\viqo`

2. Starte viqo neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt viqo, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
viqo erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
