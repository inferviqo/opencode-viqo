...
description: Cree herramientas que LLM pueda llamar en viqo.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de viqo como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.viqo/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/viqo/tools/`.
...
```ts title=".viqo/tools/database.ts" {1}
...
```ts title=".viqo/tools/project.ts" {8}
import { tool } from "@viqo-ai/plugin"
...
```python title=".viqo/tools/add.py"
import sys
...
```ts title=".viqo/tools/python-add.ts" {10}
import { tool } from "@viqo-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".viqo/tools/add.py")
