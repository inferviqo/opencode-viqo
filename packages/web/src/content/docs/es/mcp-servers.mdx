...
Puede agregar herramientas externas a Viqo utilizando el _Model Context Protocol_ o MCP. Viqo admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Viqo Config](https://inferviqo.com/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/viqo`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Viqo maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Viqo:
...
Si el servidor requiere autenticación, Viqo le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `viqo mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Viqo almacenará los tokens de forma segura en `~/.local/share/viqo/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Viqo, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Viqo como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Viqo a su cuenta Sentry.
