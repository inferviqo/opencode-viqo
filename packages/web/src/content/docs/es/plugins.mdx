---
title: Complementos
description: Escriba sus propios complementos para extender Viqo.
---

Los complementos le permiten extender Viqo al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Viqo.
...
- `.viqo/plugins/` - Complementos a nivel de proyecto
- `~/.config/viqo/plugins/` - Complementos globales
...
```json title="viqo.json"
{
  ...
  "plugin": ["viqo-helicone-session", "viqo-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/viqo/node_modules/`.
...
1. Configuración global (`~/.config/viqo/viqo.json`)
2. Configuración del proyecto (`viqo.json`)
3. Directorio global de complementos (`~/.config/viqo/plugins/`)
4. Directorio de complementos del proyecto (`.viqo/plugins/`)
...
Viqo ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".viqo/plugins/my-plugin.ts"
...
```js title=".viqo/plugins/example.js"
...
- `client`: Un cliente SDK viqo para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar viqo.
...
```js title=".viqo/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "viqo"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Viqo, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que viqo lea archivos `.env`:

```javascript title=".viqo/plugins/env-protection.js"
...
```javascript title=".viqo/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a viqo:

```ts title=".viqo/plugins/custom-tools.ts"
import { type Plugin, tool } from "@viqo-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que viqo puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para viqo junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://inferviqo.com/docs/sdk) para obtener más detalles.
...
```ts title=".viqo/plugins/compaction.ts"
import type { Plugin } from "@viqo-ai/plugin"
...
```ts title=".viqo/plugins/custom-compaction.ts"
import type { Plugin } from "@viqo-ai/plugin"
