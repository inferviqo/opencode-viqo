---
title: Servidor
description: Interactuar con el servidor viqo a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `viqo serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente viqo puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `viqo.local` |
...
Configure `VIQO_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `viqo`, o configure `VIQO_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `viqo serve` como a `viqo web`.
...
Cuando ejecuta `viqo`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor viqo para interactuar con viqo mediante programación.
:::

Esta arquitectura permite que viqo admita múltiples clientes y le permite interactuar con viqo mediante programación.

Puede ejecutar `viqo serve` para iniciar un servidor independiente. Si tienes el
viqo TUI ejecutándose, `viqo serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Viqo [IDE](/docs/ide).
...
El servidor viqo expone las siguientes API.
