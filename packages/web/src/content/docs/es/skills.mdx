...
Las habilidades del agente permiten a Viqo descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Viqo busca estas ubicaciones:

- Configuración del proyecto: `.viqo/skills/<name>/SKILL.md`
- Configuración global: `~/.config/viqo/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Viqo sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.viqo/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.viqo/skills/git-release/SKILL.md` así:
...
Viqo enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `viqo.json`:
...
**Para agentes integrados** (en `viqo.json`):
