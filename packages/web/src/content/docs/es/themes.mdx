...
Con Viqo puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Viqo usa nuestro propio tema `viqo`.
...
Viqo viene con varios temas integrados.
...
- Quiere que Viqo coincida con la apariencia de su terminal
...
Viqo admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/viqo/themes/*.json` o `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.viqo/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.viqo/themes/*.json`
