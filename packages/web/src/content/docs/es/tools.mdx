...
Las herramientas permiten que LLM realice acciones en su código base. Viqo viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Viqo.
...
:::note
Esta herramienta solo está disponible cuando `VIQO_EXPERIMENTAL_LSP_TOOL=true` (o `VIQO_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Viqo o cuando la variable de entorno `VIQO_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Viqo:

```bash
VIQO_ENABLE_EXA=1 viqo
```
