---
title: TUI
description: Usando la interfaz de usuario del terminal Viqo.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Viqo proporciona una interfaz de terminal interactiva o TUI para trabajar en sus proyectos con un LLM.

Al ejecutar Viqo se inicia TUI para el directorio actual.
...
Cuando utilice Viqo TUI, puede escribir `/` seguido de un nombre de comando para ejecutar acciones rápidamente. Por ejemplo:
...
Agregue un proveedor a Viqo. Le permite seleccionar entre proveedores disponibles y agregar sus claves API.
...
Esto es independiente de `viqo.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
- `leader_timeout`: controla cuánto tiempo espera Viqo después de la leader key. El valor predeterminado es `2000`.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `viqo.default`.
