---
title: Plugins
description: Écrivez vos propres plugins pour étendre Viqo.
---

Les plugins vous permettent d'étendre Viqo en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Viqo.
...
- `.viqo/plugins/` - Plugins au niveau du projet
- `~/.config/viqo/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/viqo/node_modules/`.
...
1. Configuration globale (`~/.config/viqo/viqo.json`)
2. Configuration du projet (`viqo.json`)
3. Répertoire global des plugins (`~/.config/viqo/plugins/`)
4. Répertoire des plugins du projet (`.viqo/plugins/`)
...
Viqo exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK viqo pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre viqo.
...
:::note
Si vous utilisez l'application de bureau Viqo, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez viqo de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à viqo :
...
L'assistant `tool` crée un outil personnalisé que viqo peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour viqo aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://inferviqo.com/docs/sdk) pour plus de détails.
