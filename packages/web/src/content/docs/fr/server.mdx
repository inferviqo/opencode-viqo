---
title: Serveur
description: Interagissez avec le serveur viqo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `viqo serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client viqo peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `viqo.local` |
...
Définissez `VIQO_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `viqo`, ou définissez `VIQO_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `viqo serve` et à `viqo web`.
...
Lorsque vous exécutez `viqo`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur viqo pour interagir avec viqo par programmation.
:::

Cette architecture permet à viqo de prendre en charge plusieurs clients et vous permet d'interagir avec viqo par programme.

Vous pouvez exécuter `viqo serve` pour démarrer un serveur autonome. Si vous avez le
viqo TUI en cours d'exécution, `viqo serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Viqo [IDE](/docs/ide).
...
Le serveur viqo expose les API suivantes.
