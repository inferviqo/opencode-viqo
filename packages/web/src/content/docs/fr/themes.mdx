...
Avec Viqo, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Viqo utilise notre propre thème `viqo`.
...
Viqo est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Viqo corresponde à l'apparence de leur terminal
...
Viqo prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/viqo/themes/*.json` ou `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.viqo/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.viqo/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/viqo/themes
vim ~/.config/viqo/themes/my-theme.json
```
...
{
  "$schema": "https://inferviqo.com/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
