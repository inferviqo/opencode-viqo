...
Puoi aggiungere strumenti esterni a Viqo usando il _Model Context Protocol_, o MCP. Viqo supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Viqo Config](https://inferviqo.com/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/viqo`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Viqo gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Viqo:
...
Se il server richiede autenticazione, Viqo ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `viqo mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Viqo memorizzera' i token in modo sicuro in `~/.local/share/viqo/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Viqo insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Viqo come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Viqo al tuo account Sentry.
