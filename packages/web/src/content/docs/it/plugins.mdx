---
title: Plugin
description: Scrivi plugin per estendere Viqo.
---

I plugin ti permettono di estendere Viqo agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Viqo.
...
- `.viqo/plugins/` - plugin a livello progetto
- `~/.config/viqo/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/viqo/node_modules/`.
...
1. Config globale (`~/.config/viqo/viqo.json`)
2. Config di progetto (`viqo.json`)
3. Directory plugin globale (`~/.config/viqo/plugins/`)
4. Directory plugin di progetto (`.viqo/plugins/`)
...
Viqo esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di viqo per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere viqo.
...
:::note
Se usi l'app desktop di Viqo, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a viqo di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a viqo:
...
L'helper `tool` crea uno strumento personalizzato che viqo può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in viqo insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://inferviqo.com/docs/sdk) per i dettagli.
