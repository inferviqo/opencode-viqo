---
title: Server
description: Interagisci con il server viqo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `viqo serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client viqo.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `viqo.local` |
...
Imposta `VIQO_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `viqo`, oppure imposta `VIQO_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `viqo serve` sia per `viqo web`.
...
Quando esegui `viqo` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server viqo per interagire con viqo in modo programmatico.
:::

Questa architettura permette a viqo di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `viqo serve` per avviare un server standalone. Se la TUI di viqo e gia in esecuzione, `viqo serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Viqo.
...
Il server viqo espone le seguenti API.
