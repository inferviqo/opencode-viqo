...
Le skill degli agenti permettono a Viqo di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Viqo cerca in queste posizioni:

- Config di progetto: `.viqo/skills/<name>/SKILL.md`
- Config globale: `~/.config/viqo/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Viqo risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.viqo/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.viqo/skills/git-release/SKILL.md` cosi':
...
Viqo elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `viqo.json`:
...
**Per agenti integrati** (in `viqo.json`):
