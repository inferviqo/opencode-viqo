---
title: サーバー
description: HTTP 経由で Viqo サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`viqo serve` コマンドは、Viqo クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `viqo.local` |
...
HTTP 基本認証でサーバーを保護するには、`VIQO_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `viqo` になるか、`VIQO_SERVER_USERNAME` を設定してオーバーライドします。これは、`viqo serve` と `viqo web` の両方に当てはまります。
...
`viqo` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Viqo サーバーを使用して、プログラムで Viqo と対話します。
:::

`viqo serve` を実行してスタンドアロンサーバーを起動できます。すでに Viqo TUI を実行している場合でも、`viqo serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Viqo [IDE](/docs/ide) プラグインによって使用されます。
