...
You can add external tools to Viqo using the _Model Context Protocol_, or MCP. Viqo supports both local and remote servers.
...
You can define MCP servers in your [Viqo Config](https://inferviqo.com/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/viqo` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Viqo automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Viqo will:
...
If the server requires authentication, Viqo will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `viqo mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Viqo will store the tokens securely in `~/.local/share/viqo/mcp-auth.json`.
...
Your MCPs are available as tools in Viqo, alongside built-in tools. So you can manage them through the Viqo config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Viqo to your Sentry account.
