...
Du kan legge til eksterne verktøy i Viqo ved å bruke _Model Context Protocol_ eller MCP. Viqo støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Viqo Config](https://inferviqo.com/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="viqo.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/viqo`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Viqo håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Viqo:
...
Hvis serveren krever autentisering, vil Viqo be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `viqo mcp auth <server-name>`.
...
```bash
viqo mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Viqo lagre tokens sikkert i `~/.local/share/viqo/mcp-auth.json`.
...
viqo mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Viqo, sammen med innebygde verktøy. Så du kan administrere dem gjennom Viqo-konfigurasjonen som alle andre verktøy.
...
```bash
viqo mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Viqo til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Viqo-samtale oppført som en separat økt.
