...
description: Skriv dine egne programtillegg for å utvide Viqo.
---

Plugins lar deg utvide Viqo ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Viqo.
...
- `.viqo/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/viqo/node_modules/`.
...
1. Global konfigurasjon (`~/.config/viqo/viqo.json`)
2. Prosjektkonfigurasjon (`viqo.json`)
3. Global plugin-katalog (`~/.config/viqo/plugins/`)
4. Prosjektpluginkatalog (`.viqo/plugins/`)
...
Viqo kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Viqo SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Viqo.
...
:::note
Hvis du bruker Viqo-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Viqo fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Viqo:
...
`tool`-hjelperen lager et tilpasset verktøy som Viqo kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Viqo sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://inferviqo.com/docs/sdk) for detaljer.
