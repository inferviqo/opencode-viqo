---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Viqo.
---

import config from "../../../../config.mjs"
export const console = config.console

Viqo bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Viqo-konfigurasjonen.
...
i `~/.local/share/viqo/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Viqo
konfig.
...
Viqo Zen er en liste over modeller levert av Viqo-teamet som har vært
testet og verifisert for å fungere godt med Viqo. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Viqo Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Viqo Zen` og gå til [inferviqo.com/auth](https://inferviqo.com/zen).
...
Det fungerer som alle andre leverandører i Viqo og er helt valgfritt å bruke.

---

## Viqo Go

Viqo Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Viqo-teamet som har vært
testet og verifisert for å fungere godt med Viqo.

1. Kjør kommandoen `/connect` i TUI, velg `Viqo Go`, og gå til [inferviqo.com/auth](https://inferviqo.com/zen).
...
Slik bruker du Amazon Bedrock med Viqo:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Viqo:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `viqo.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i viqo støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere viqo til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Viqo skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Viqo-konfigurasjonen.
...
for å lagre token i Viqo auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Viqo bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Viqo
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`viqo.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Viqo:
...
Slik bruker du Google Vertex AI med Viqo:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Viqo, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session) plugin for automatisk å logge hver Viqo-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Viqo-samtale oppført som en separat økt.
...
Du kan konfigurere Viqo for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Viqo for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Viqo for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Viqo. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/viqo) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Viqo:
...
3. Kopier API-nøkkelen for bruk i Viqo.
...
6. **Viktig**: Før du bruker skymodeller i Viqo, må du hente modellinformasjonen lokalt:
...
Viqo Zen er en liste over testede og verifiserte modeller levert av Viqo-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Viqo Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Viqo Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Viqo API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Viqo-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Viqo-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Viqo:
...
Du kan også tilpasse modeller gjennom Viqo-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Viqo. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `viqo.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Viqo forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `viqo auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Viqo-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Viqo-konfigurasjonen.
