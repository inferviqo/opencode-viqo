---
title: Tjener
description: Samhandle med Viqo-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `viqo serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Viqo-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `viqo.local` |
...
Sett `VIQO_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `viqo` som standard, eller sett `VIQO_SERVER_USERNAME` for å overstyre det. Dette gjelder både `viqo serve` og `viqo web`.
...
Når du kjører `viqo` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Viqo-serveren til å samhandle med Viqo programmatisk.
:::

Denne arkitekturen lar Viqo støtte flere klienter og lar deg samhandle med Viqo programmatisk.

Du kan kjøre `viqo serve` for å starte en frittstående server. Hvis du har
Viqo TUI kjører, vil `viqo serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Viqo [IDE](/docs/ide) plugins.
...
viqo-serveren viser følgende APIer.
