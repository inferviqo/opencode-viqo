...
Viqo uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Viqo with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
viqo --auto
```

You can also use auto mode with [`viqo run`](/docs/cli#run).

```bash
viqo run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Viqo was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Viqo permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Viqo starts from permissive defaults:
...
When Viqo prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Viqo session)
