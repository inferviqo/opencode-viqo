...
Możesz dodać dodatkowe informacje do viqo za pomocą _Model Context Protocol_, w skrócie MCP. viqo obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [viqo Config](https://inferviqo.com/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/viqo`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
viqo automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, viqo:
...
Jeśli serwer wymaga uwierzytelnienia, viqo poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `viqo mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji viqo bezpieczne przechowa tokeny w `~/.local/share/viqo/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w viqo, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie viqo, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć viqo z kontem Sentry.
