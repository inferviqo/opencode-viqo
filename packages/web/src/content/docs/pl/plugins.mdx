---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć viqo.
---

Wtyczki umożliwiające wyłączenie viqo poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania viqo.
...
- `.viqo/plugins/` - Wtyczki na poziomie projektu
- `~/.config/viqo/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/viqo/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/viqo/viqo.json`)
2. Konfiguracja projektu (`viqo.json`)
3. Globalny katalog wtyczek (`~/.config/viqo/plugins/`)
4. Katalog wtyczek projektu (`.viqo/plugins/`)
...
viqo uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z viqo do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do viqo.
...
:::note
Jeśli korzystasz z aplikacji komputerowej viqo, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj viqo czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do viqo:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod viqo. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://inferviqo.com/docs/sdk).
