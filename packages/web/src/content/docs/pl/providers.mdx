---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w viqo.
---

import config from "../../../../config.mjs"
export const console = config.console

viqo używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji viqo.
...
w `~/.local/share/viqo/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim viqo
config.
...
Viqo Zen to lista modeli dostarczonych przez zespół viqo, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z viqo. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Viqo Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Viqo Zen` i przejdź do [inferviqo.com/auth](https://inferviqo.com/zen).
...
Działa jak każdy inny dostawca w viqo i jest całkowicie opcjonalny w użyciu.

---

## Viqo Go

Viqo Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół viqo, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z viqo.

1. Uruchom polecenie `/connect` w TUI, wybierz `Viqo Go` i przejdź do [inferviqo.com/auth](https://inferviqo.com/zen).
...
Aby używać Amazon Bedrock z viqo:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania viqo:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `viqo.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w viqo nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować viqo do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod viqo działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji viqo.
...
viqo używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować viqo
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`viqo.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem viqo:
...
Aby używać Google Vertex AI z viqo:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania viqo.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą viqo, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session), aby automatycznie rejestrować każdą konwersację viqo jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja viqo będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować viqo tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować viqo tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować viqo tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem viqo. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/viqo).
:::
...
Aby korzystać z Ollama Cloud z viqo:
...
3. Skopiuj klucz API do użycia w viqo.
...
6. **Ważne**: Przed użyciem modeli chmurowych w viqo musisz pobrać informacje o modelu lokalnie:
...
Viqo Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół viqo. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Viqo Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Viqo Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API viqo.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji viqo.
...
5. Możesz je także dostosować za pomocą konfiguracji viqo. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z viqo:
...
Możesz także dostosować modele za pomocą konfiguracji viqo. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z viqo. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `viqo.json` w katalogu projektu:
...
Pola `limit` pozwalają viqo zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `viqo auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację viqo i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji viqo.
