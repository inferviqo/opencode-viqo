---
title: Serwer
description: Interakcja z serwerem viqo za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `viqo serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient viqo.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `viqo.local` |
...
Ustaw `VIQO_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `viqo` lub ustaw `VIQO_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `viqo serve`, jak i `viqo web`.
...
Kiedy uruchomisz `viqo`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera viqo do programowej interakcji z viqo.
:::

Ta architektura umożliwia obsługę wielu klientów przez viqo i programową interakcję z kodem otwartym.

Możesz uruchomić `viqo serve`, aby uruchomić samodzielny serwer. Jeśli masz
viqo TUI działa, `viqo serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki viqo [IDE](/docs/ide).
...
Serwer viqo udostępnia następujące interfejsy API.
