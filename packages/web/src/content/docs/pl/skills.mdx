...
Umiejętności agenta pozwalają viqo odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
viqo przeszukuje te lokalizacje:

- Project config: `.viqo/skills/<name>/SKILL.md`
- Global config: `~/.config/viqo/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu viqo przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.viqo/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.viqo/skills/git-release/SKILL.md` w ten sposób:
...
viqo wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `viqo.json`:
...
**Dla agentów wbudowanych** (w `viqo.json`):
