...
Dzięki viqo możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie viqo używa naszego własnego motywu `viqo`.
...
viqo ma kilka wbudowanych motywów.
...
- Chcą, aby Viqo pasował do wyglądu ich terminala
...
viqo obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/viqo/themes/*.json` lub `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.viqo/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.viqo/themes/*.json`
