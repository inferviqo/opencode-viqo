...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Viqo posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Viqo.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `VIQO_EXPERIMENTAL_LSP_TOOL=true` (lub `VIQO_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Viqo lub gdy zmienna środowiskowa `VIQO_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Viqo:
