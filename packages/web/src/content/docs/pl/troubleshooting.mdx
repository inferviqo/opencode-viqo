...
Aby debugować problemy z viqo, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\viqo\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `viqo --log-level DEBUG`.

---

## Przechowywanie danych

viqo przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\viqo`
...
viqo Desktop uruchamia w tle lokalny serwer viqo (sidecar `viqo-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Viqo` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (lub `~/.config/viqo/viqo.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\viqo\viqo.jsonc`
...
viqo może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\viqo\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby viqo mógł ją odbudować.

1. Całkowicie zamknij viqo Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/viqo`
- **Linux**: usuń `~/.cache/viqo` (lub uruchom `rm -rf ~/.cache/viqo`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\viqo`

3. Uruchom ponownie viqo Desktop.

---

### Napraw problemy z połączeniem z serwerem

viqo Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `viqo.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `VIQO_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `VIQO_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows viqo Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami viqo.

---

### Brak powiadomień

viqo Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla viqo w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie viqo Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji viqo Desktop):

- `viqo.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `viqo.global.dat` i `viqo.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `viqo upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `viqo models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\viqo`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. viqo dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\viqo`

2. Uruchom ponownie kod viqo, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to viqo do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
viqo wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
