---
title: Providers
description: Using any LLM provider in Viqo.
---

import config from "../../../config.mjs"
export const console = config.console

Viqo uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Viqo config.
...
in `~/.local/share/viqo/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Viqo
config.
...
Viqo Zen is a list of models provided by the Viqo team that have been
tested and verified to work well with Viqo. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Viqo Zen.
:::

1. Run the `/connect` command in the TUI, select `Viqo Zen`, and head to [inferviqo.com/auth](https://inferviqo.com/zen).
...
It works like any other provider in Viqo and is completely optional to use.

---

## Viqo Go

Viqo Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Viqo team that have been
tested and verified to work well with Viqo.

1. Run the `/connect` command in the TUI, select `Viqo Go`, and head to [inferviqo.com/auth](https://inferviqo.com/zen).
...
To use Amazon Bedrock with Viqo:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running viqo:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY viqo

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile viqo

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX viqo
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `viqo.json`:

   ```json title="viqo.json"
   {
     "$schema": "https://inferviqo.com/config.json",
     "provider": {
       "amazon-bedrock": {
...
Viqo. Anthropic explicitly prohibits this.

Previous versions of Viqo came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Viqo with zero setup:
...
You can configure viqo to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="viqo.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for viqo to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX viqo
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your viqo config.
...
Viqo supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Viqo uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Viqo. Sign in and approve.

   :::note
   Viqo requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Viqo surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Viqo forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Viqo is experimental. Features, configuration, and
behavior may change in future releases.
:::

Viqo integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in viqo auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Viqo uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Viqo
to only use your own GitLab-hosted instance, add the following to your
`viqo.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Viqo will:
...
4. Route tool execution requests through Viqo's permission-gated tool system
...
To use your GitHub Copilot subscription with viqo:
...
To use Google Vertex AI with Viqo:
...
2. Set the required environment variables:
   ...
   Set them while running viqo.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id viqo
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through viqo, you can always configure it yourself.
...
```jsonc title="~/.config/viqo/viqo.jsonc"
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session) plugin to automatically log each Viqo conversation as a session in Helicone.

```bash
npm install -g viqo-helicone-session
```
...
```json title="viqo.json"
{
  "plugin": ["viqo-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Viqo conversation listed as a separate session.
...
You can configure viqo to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="viqo.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure viqo to use local models through LM Studio.

```json title="viqo.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "lmstudio": {
...
You can configure viqo to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Viqo. See the [Ollama integration docs](https://docs.ollama.com/integrations/viqo) for details.
:::

```json title="viqo.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "ollama": {
...
3. Copy the API key for use in Viqo.
...
6. **Important**: Before using cloud models in Viqo, you must pull the model information locally:
...
Viqo Zen is a list of tested and verified models provided by the Viqo team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Viqo Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Viqo Zen**.

   ```txt
   /connect
   ```

3. Enter your Viqo API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your viqo config.
...
5. You can also customize them through your viqo config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' viqo
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group viqo
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=viqo&utm_medium=docs&utm_campaign=cortex-provider).

viqo's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Viqo captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting viqo:
...
The model catalog is provided automatically. A minimal `viqo.json` is all that's needed:
...
You can also customize models through your viqo config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Viqo opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Viqo refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Viqo is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Viqo — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Viqo prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Viqo polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with viqo. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in viqo.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `viqo.json` file in your project directory:

   ```json title="viqo.json" ""myprovider"" {5-15}
   {
     "$schema": "https://inferviqo.com/config.json",
     "provider": {
       "myprovider": {
...
```json title="viqo.json" {9,11,17-20}
{
  "$schema": "https://inferviqo.com/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Viqo to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `viqo auth list` to see if the credentials
...
2. For custom providers, check the viqo config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your viqo config.
