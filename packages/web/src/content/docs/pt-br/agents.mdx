...
Existem dois tipos de agentes no viqo; agentes primários e subagentes.
...
viqo vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Viqo vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Viqo vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Viqo, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `viqo.json`:
...
- Global: `~/.config/viqo/agents/`
- Por projeto: `.viqo/agents/`

```markdown title="~/.config/viqo/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o viqo usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do viqo quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do viqo usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Viqo Zen](/docs/zen), você usaria `viqo/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `viqo models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/Inferviqo/viqo).
:::
...
```markdown title="~/.config/viqo/agents/docs-writer.md"
---
...
```markdown title="~/.config/viqo/agents/security-auditor.md"
---
