...
description: Crie ferramentas que o LLM pode chamar no viqo.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do viqo, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.viqo/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/viqo/tools/`.
