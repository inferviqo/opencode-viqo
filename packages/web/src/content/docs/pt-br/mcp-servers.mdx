...
Você pode adicionar ferramentas externas ao viqo usando o _Model Context Protocol_, ou MCP. O viqo suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do viqo](https://inferviqo.com/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/viqo`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O viqo lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o viqo irá:
...
Se o servidor requer autenticação, o viqo solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `viqo mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o viqo armazenará os tokens de forma segura em `~/.local/share/viqo/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no viqo, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do viqo como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o viqo à sua conta do Sentry.
