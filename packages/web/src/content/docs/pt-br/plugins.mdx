---
title: Plugins
description: Escreva seus próprios plugins para estender o viqo.
---

Plugins permitem que você estenda o viqo conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do viqo.
...
- `.viqo/plugins/` - Plugins em nível de projeto
- `~/.config/viqo/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/viqo/node_modules/`.
...
1. Configuração global (`~/.config/viqo/viqo.json`)
2. Configuração do projeto (`viqo.json`)
3. Diretório de plugins global (`~/.config/viqo/plugins/`)
4. Diretório de plugins do projeto (`.viqo/plugins/`)
...
O viqo executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do viqo para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o viqo.
...
:::note
Se você estiver usando o aplicativo desktop viqo, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o viqo de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao viqo:
...
O helper `tool` cria uma ferramenta personalizada que o viqo pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o viqo junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://inferviqo.com/docs/sdk) para detalhes.
