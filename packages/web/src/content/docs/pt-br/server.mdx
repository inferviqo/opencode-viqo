---
title: Servidor
description: Interaja com o servidor viqo via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `viqo serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente viqo pode usar.
...
```bash
viqo serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `viqo.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
viqo serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `VIQO_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `viqo`, ou defina `VIQO_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `viqo serve` quanto ao `viqo web`.

```bash
VIQO_SERVER_PASSWORD=your-password viqo serve
```
...
Quando você executa `viqo`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor viqo para interagir com o viqo programaticamente.
:::

Essa arquitetura permite que o viqo suporte múltiplos clientes e permite que você interaja com o viqo programaticamente.

Você pode executar `viqo serve` para iniciar um servidor autônomo. Se você tiver o TUI do viqo em execução, `viqo serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do viqo [IDE](/docs/ide).
...
O servidor viqo expõe as seguintes APIs.
