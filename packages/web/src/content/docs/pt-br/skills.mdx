...
As habilidades do agente permitem que o viqo descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O viqo pesquisa nesses locais:

- Configuração do projeto: `.viqo/skills/<name>/SKILL.md`
- Configuração global: `~/.config/viqo/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o viqo sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.viqo/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/viqo/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.viqo/skills/git-release/SKILL.md` assim:
...
O viqo lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `viqo.json`:
...
**Para agentes embutidos** (em `viqo.json`):
