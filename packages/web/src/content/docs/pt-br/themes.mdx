...
Com o viqo, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o viqo usa nosso próprio tema `viqo`.
...
O viqo vem com vários temas embutidos.
...
- Querem que o viqo corresponda à aparência do seu terminal
...
O viqo suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/viqo/themes/*.json` ou `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.viqo/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.viqo/themes/*.json`
