...
As ferramentas permitem que o LLM execute ações em sua base de código. O viqo vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no viqo.
...
:::note
Esta ferramenta está disponível apenas quando `VIQO_EXPERIMENTAL_LSP_TOOL=true` (ou `VIQO_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor viqo ou quando a variável de ambiente `VIQO_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o viqo:
