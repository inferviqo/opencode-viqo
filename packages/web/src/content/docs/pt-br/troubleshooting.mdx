...
Para depurar problemas com o viqo, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\viqo\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `viqo --log-level DEBUG`.

---

## Armazenamento

O viqo armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\viqo`
...
O viqo Desktop executa um servidor viqo local (o sidecar `viqo-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Viqo` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (ou `~/.config/viqo/viqo.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\viqo\viqo.jsonc`
...
O viqo também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\viqo\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o viqo possa reconstruí-lo.

1. Saia completamente do viqo Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/viqo`
- **Linux**: exclua `~/.cache/viqo` (ou execute `rm -rf ~/.cache/viqo`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\viqo`

3. Reinicie o viqo Desktop.

---

### Corrigindo problemas de conexão com o servidor

O viqo Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `viqo.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `VIQO_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `VIQO_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o viqo Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do viqo.

---

### Notificações não aparecendo

O viqo Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o viqo nas configurações do seu sistema operacional, e
...
1. Saia do viqo Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo viqo Desktop):

- `viqo.settings.dat` (URL do servidor padrão do desktop)
- `viqo.global.dat` e `viqo.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o viqo:
...
3. Certifique-se de que você tem a versão mais recente com `viqo upgrade`
...
Para descobrir quais modelos você tem acesso, execute `viqo models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\viqo`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O viqo instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\viqo`

2. Reinicie o viqo para reinstalar os pacotes de provedores mais recentes

Isso forçará o viqo a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O viqo detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
