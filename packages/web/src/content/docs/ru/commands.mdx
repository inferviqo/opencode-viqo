...
Создайте `.viqo/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию viqo или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем viqo [config](/docs/config):
...
- Глобальный: `~/.config/viqo/commands/`
- Для каждого проекта: `.viqo/commands/`
...
viqo включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
