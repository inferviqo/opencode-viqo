---
title: Server
description: Interact with viqo server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `viqo serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an viqo client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `viqo.local` |
...
Set `VIQO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `viqo`, or set `VIQO_SERVER_USERNAME` to override it. This applies to both `viqo serve` and `viqo web`.
...
When you run `viqo` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the viqo server to interact with viqo programmatically.
:::

This architecture lets viqo support multiple clients and allows you to interact with viqo programmatically.

You can run `viqo serve` to start a standalone server. If you have the
viqo TUI running, `viqo serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Viqo [IDE](/docs/ide) plugins.
...
The viqo server exposes the following APIs.
