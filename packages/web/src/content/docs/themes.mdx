...
With Viqo you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Viqo uses our own `viqo` theme.
...
Viqo comes with several built-in themes.
...
- Want Viqo to match their terminal's appearance
...
Viqo supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/viqo/themes/*.json` or `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.viqo/themes/*.json`
4. **Current working directory** - Defined in `./.viqo/themes/*.json`
