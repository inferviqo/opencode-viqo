...
Viqo Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Viqo'daki diğer sağlayıcılar gibi çalışır. Viqo Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Viqo'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Viqo Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Viqo Go, Viqo'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Viqo Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Viqo Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Viqo Go'ya abone olabilir.
:::
...
Viqo Go aşağıdaki limitleri içerir:
...
Viqo yapılandırmanızdaki [model id](/docs/config/#models) formatı `viqo-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `viqo-go/kimi-k2.7-code` kullanmalısınız.
...
Viqo Go'yu şu amaçlarla oluşturduk:
...
4. Viqo ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
