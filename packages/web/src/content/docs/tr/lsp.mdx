...
description: Viqo, LSP sunucularınızla bütünleşir.
---

Viqo, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Viqo, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`VIQO_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve viqo bir dosyayı açtığında:
...
LSP sunucularını, viqo yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
