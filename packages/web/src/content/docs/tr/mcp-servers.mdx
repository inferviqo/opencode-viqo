...
_Model Bağlam Protokolü_ veya MCP kullanarak viqo'a harici araçlar ekleyebilirsiniz. viqo hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [viqo Config](https://inferviqo.com/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/viqo` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
viqo, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde viqo şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa viqo, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `viqo mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından viqo, belirteçleri `~/.local/share/viqo/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra viqo'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi viqo yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve viqo'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
