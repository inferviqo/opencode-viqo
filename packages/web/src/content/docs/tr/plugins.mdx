---
title: Eklentiler
description: Viqo'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Viqo'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Viqo'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.viqo/plugins/` - Proje düzeyinde eklentiler
- `~/.config/viqo/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/viqo/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/viqo/viqo.json`)
2. Project config (`viqo.json`)
3. Global eklenti dizini (`~/.config/viqo/plugins/`)
4. Proje eklenti dizini (`.viqo/plugins/`)
...
Viqo bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Viqo SDK istemcisi.
...
Viqo'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Viqo masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Viqo'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Viqo'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Viqo'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Viqo'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://inferviqo.com/docs/sdk) bakın.
