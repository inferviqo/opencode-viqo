---
title: Sağlayıcılar
description: viqo'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

viqo, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı viqo yapılandırmanızda yapılandırın.
...
`~/.local/share/viqo/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları viqo'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Viqo Zen, viqo ekibi tarafından sağlanan modellerin bir listesidir.
viqo ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Viqo Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, viqo'u seçin ve [inferviqo.com/auth](https://inferviqo.com/auth)'ye gidin.
...
viqo'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Viqo Go

Viqo Go, viqo ile iyi çalıştığı test edilmiş ve doğrulanmış, viqo ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Viqo Go`'yu seçin ve [inferviqo.com/auth](https://inferviqo.com/zen) adresine gidin.
...
Amazon Bedrock'u viqo ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   viqo'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `viqo.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi viqo'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
viqo'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   viqo'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
viqo, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. viqo'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`viqo.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`viqo.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi viqo'la kullanmak için:
...
Google Vertex AI'yi viqo ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları viqo çalıştırılırken ayarlayın.
...
Helicone'un viqo aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her viqo görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [viqo-helicone-session](https://github.com/H2Shami/viqo-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her viqo görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
viqo'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla viqo'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla viqo'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini viqo için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/viqo)'a bakın.
:::
...
Ollama Cloud'u viqo ile kullanmak için:
...
3. viqo'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: viqo'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Viqo Zen, viqo ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Viqo Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Viqo Zen**'i arayın.

   ```txt
   /connect
   ```

3. viqo API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   viqo yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca viqo yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri viqo yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı viqo ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `viqo.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları viqo'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `viqo auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için viqo yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, viqo yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
