...
viqo ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak viqo kendi `viqo` temasını kullanır.
...
viqo birden fazla yerleşik temayla gelir.
...
- viqo'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
viqo, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/viqo/themes/*.json` veya `$XDG_CONFIG_HOME/viqo/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.viqo/themes/*.json`
4. **Geçerli çalışma dizini** - `./.viqo/themes/*.json`
