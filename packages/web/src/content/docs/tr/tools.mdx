...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. viqo yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
viqo içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `VIQO_EXPERIMENTAL_LSP_TOOL=true` (veya `VIQO_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca viqo provider kullanılırken veya `VIQO_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

viqo başlatırken etkinleştirmek için:
