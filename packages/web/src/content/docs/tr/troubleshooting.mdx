...
viqo ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\viqo\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `viqo --log-level DEBUG`.

---

## Depolama

viqo, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/viqo/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\viqo` yapistirin
...
viqo Desktop arka planda yerel bir viqo sunucusu (`viqo-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `viqo` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/viqo/viqo.jsonc` (veya `~/.config/viqo/viqo.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/viqo/viqo.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\viqo\viqo.jsonc` yapistirin
...
viqo diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/viqo/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\viqo\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), viqo'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. viqo Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/viqo`
- **Linux**: `~/.cache/viqo` dizinini silin (veya `rm -rf ~/.cache/viqo` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\viqo` yapistirin

3. viqo Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

viqo Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`viqo.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `VIQO_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `VIQO_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta viqo Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, viqo ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

viqo Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda viqo icin bildirimler etkinse
...
1. viqo Desktop'u kapatin
2. Su dosyalari bulun ve silin (viqo Desktop uygulama veri dizininde yer alirlar):

- `viqo.settings.dat` (desktop varsayilan sunucu URL'si)
- `viqo.global.dat` ve `viqo.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
viqo ile ilgili bir sorun yasiyorsaniz:
...
3. `viqo upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `viqo models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\viqo`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. viqo, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\viqo`

2. En guncel provider paketlerini yeniden kurmak icin viqo'u yeniden baslatin

Bu, viqo'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
viqo, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
