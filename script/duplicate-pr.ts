...
import { createOpencode } from "@viqo-ai/sdk"
...
`)
  ...
  const viqo = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await viqo.client.session.create()
    const result = await viqo.client.session
      .prompt({
