# Remove `packages/viqo/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/viqo/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the viqo legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/viqo/src/storage/db.ts` are concentrated in 22 source files:

- `packages/viqo/src/account/repo.ts`
- `packages/viqo/src/cli/cmd/db.ts`
- `packages/viqo/src/cli/cmd/import.ts`
- `packages/viqo/src/cli/cmd/stats.ts`
- `packages/viqo/src/control-plane/workspace.ts`
- `packages/viqo/src/index.ts`
- `packages/viqo/src/node.ts`
- `packages/viqo/src/permission/index.ts`
- `packages/viqo/src/project/project.ts`
- `packages/viqo/src/server/projectors.ts`
- `packages/viqo/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/viqo/src/server/shared/fence.ts`
- `packages/viqo/src/session/message-v2.ts`
- `packages/viqo/src/session/projectors.ts`
- `packages/viqo/src/session/prompt.ts`
- `packages/viqo/src/session/session.ts`
- `packages/viqo/src/session/todo.ts`
- `packages/viqo/src/share/share-next.ts`
- `packages/viqo/src/storage/db.ts`
- `packages/viqo/src/sync/index.ts`
- `packages/viqo/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy viqo database wrapper; `packages/viqo/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the viqo wrapper.
...
Status: Completed. `SyncEvent` and the viqo projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy viqo database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy viqo database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy viqo database wrapper.
...
All migration groups are complete or superseded. `packages/viqo/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No viqo data-migration group remains.

The previous viqo `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate viqo data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/viqo`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/viqo/src`
- `bun typecheck` from `packages/viqo`
- Relevant package tests from `packages/viqo`, not the repo root
