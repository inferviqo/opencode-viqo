...
Move the canonical Viqo terminal application from
`packages/viqo/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/viqo`,
`packages/cli`, or `@viqo-ai/core`.

The SDK is the TUI's Viqo boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/viqo`.
- Do not replace private `packages/viqo` imports with `@viqo-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@viqo-ai/tui`.
...
- Viqo domain data displayed by the TUI
...
```text
packages/viqo/src/cli/cmd/tui
```

Its private dependency on `packages/viqo` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/viqo/src/*`.
...
Status: Completed. The private `@viqo-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@viqo-ai/tui`.
...
- The package has no dependency on `viqo`, `@viqo-ai/cli`, or
  `@viqo-ai/core`.
...
primitives now live in `@viqo-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@viqo-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Viqo globals or process state;
...
Replace process-global Viqo state with resolved TUI inputs.
...
`@viqo-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/viqo` initially.
...
canonical in `@viqo-ai/tui`. Configured references resolve through the new
...
adapters rather than Viqo domain implementations.

Make the SDK the only Viqo domain boundary used by the TUI.
...
- Domain-facing TUI code imports Viqo data and operations only from
  `@viqo-ai/sdk`.
...
`@viqo-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/viqo`.
...
- The package has no imports from `packages/viqo`, `packages/cli`, or
  `@viqo-ai/core`.
...
public `@viqo-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/viqo`.
- Keep the legacy embedded worker and server startup in `packages/viqo`.
- Change those adapters to load config, create transport inputs, and call the
  public `@viqo-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/viqo`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@viqo-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@viqo-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Viqo domain state.
...
cd packages/viqo && bun typecheck
...
rg '@viqo-ai/core|packages/viqo|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/viqo/src packages/viqo/test
```
...
- Build the current-platform `packages/viqo` binary.
