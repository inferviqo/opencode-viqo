...
Both `action` and `resource` use viqo's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.viqo` files is intentionally deferred until `.viqo` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether viqo permits an operation through its managed execution paths.
