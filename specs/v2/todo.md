...
The viqo server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@viqo-ai/llm` tool loop after replacing its
...
- plugins get global "viqo" instance like in that post i showed
- viqo instance has stuff like `viqo.session.prompt()` or
  `viqo.tool.register({...})`
...
- decide whether to preserve deprecated `@viqo-ai/llm` orchestration exports
